<template>
    <div>
                ...
                <el-form :inline="true" :model="searchForm" size="default">
                    ...
                    <el-form-item label="Publisher">
                        <SigleSelect url="/sysUser/list" class="search-input" columnName="userName" :clearable="true"
                            columnValue="id" v-model="searchForm.publisherId">
                        </SigleSelect>
                    </el-form-item>


                    <el-form-item label="Publish Time" class="search-input">
                        <el-date-picker v-model="searchForm.publishTimeRange" type="datetimerange"
                            start-placeholder="Start publish time" end-placeholder="End publish time"
                            value-format="YYYY-MM-DD HH:mm:ss">
                        </el-date-picker>
                    </el-form-item>


                    <el-form-item label="Published" class="search-input">
                        <el-select style="width:180px" v-model="searchForm.isPublished" class="search-input"
                            :clearable="true" placeholder="Select publish status">
        ...
        <!-- 编辑对话框 -->
        <el-dialog :title="formData.id ? 'Edit Announcement' : 'Add Announcement'" v-model="editorShow" width="50%"
                ...
                <el-row :gutter="10" class="edit-from-body">
                    ...
                    <el-col :span="24">
                        <el-form-item label="Title" prop="title">
                            <el-input type="text" v-model="formData.title" placeholder="Enter title"
                                :clearable="true"></el-input>
                        </el-form-item>
                    </el-col>
                    ...
                    <el-col :span="24">
                        <el-form-item label="Content" prop="content">
                            <el-input type="textarea" :rows="5" v-model.trim="formData.content"
                                placeholder="Enter content" :clearable="true"></el-input>
                        </el-form-item>
                    </el-col>
                    ...
                    <el-col :span="24">
                        <el-form-item label="Publish Time" prop="publishTime">
                            <el-date-picker v-model="formData.publishTime" align="right" type="datetime"
                    ...
                    <el-col :span="24">
                        <el-form-item label="Published" prop="isPublished">
                            <el-switch v-model="formData.isPublished" :active-value="true" :inactive-value="false">
                            </el-switch>
                        </el-form-item>
                    </el-col>
                </el-row>
        ...
        <el-card class="box-card">
            ...
            <el-table v-loading="loading" :data="tableData" stripe style="width: 100%"
                ...
                <el-table-column label="Username">
                    <template #default="{ row }">
                        {{ row.publisherDto.userName }}
                    </template>
                </el-table-column>
                <el-table-column label="Publish Time">
                    <template #default="{ row }">
                        {{ row.publishTime }}
                    </template>
                </el-table-column>
                <el-table-column label="Published" width="100">
                    <template #default="{ row }">
                        <el-tag :type="row.isPublished ? 'success' : 'danger'">{{ row.isPublished ? 'Yes' : 'No'
                            }}</el-tag>
                    </template>
                </el-table-column>
...
const fetchData = async () => {
    loading.value = true;
    try {
        const { data, success } = await post('/announcement/list', {
...
const showEditModal = async (id) => {
    ...
    const { data } = await post(`/announcement/findById`, { id: id });
...
const addOrEditForm = async () => {
    ...
    await editModalForm.value.validate(async valid => {
        if (valid) {
            const { success } = await post(`/announcement/addOrEdit`, formData);
...
const showDeleteModal = async (id) => {
    ...
    try {
        ...
        const { success } = await post(`/announcement/deleteById`, { id: id });
        if (success) {
            fetchData();
...
const batchDelete = async () => {
    ...
    if (ids.length > 0) {
        try {
            ...
            const { success } = await post(`/announcement/batchDeleteByIds`, { ids: ids });
            if (success) {
                fetchData();
                ElMessage.success('Deleted successfully');
            }
