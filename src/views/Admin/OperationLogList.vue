    ...
    <div>
        ...
        <el-card class="box-card">
            <el-table v-loading="loading" :data="tableData" stripe style="width: 100%">
                ...
                <el-table-column label="Operation Content" width="140">
                    <template #default="{ row }">
                        <RichButton :content="row.operationContent"></RichButton>
                    </template>
                </el-table-column>
...
// Address select dialog reference
const ipAddressLogLatSelectToastRef = ref(null);

// Open address select dialog
const ipAddressOpenLogLatSelectToast = () => {
    ipAddressLogLatSelectToastRef.value.OpenToast(formData.latitude, formData.longitude, formData.ipAddress, (data) => {
        formData.ipAddressLatitude = data.lat;
        formData.ipAddressLongitude = data.lng;
        formData.ipAddress = data.fullAddress;
        formData.ipAddressProviceCityArea = data.province + " " + data.city + " " + data.district;
    });
};
...
const fetchData = async () => {
    loading.value = true;
    try {
        const { data, success } = await post('/operationLog/list', {
...
const showEditModal = async (id) => {
    const { data } = await post(`/operationLog/findById`, { id: id });
...
const addOrEditForm = async () => {
    ...
    await editModalForm.value.validate(async valid => {
        if (valid) {
            const { success } = await post(`/operationLog/addOrEdit`, formData);
...
const showDeleteModal = async (id) => {
    try {
        ...
        const { success } = await post(`/operationLog/deleteById`, { id: id });
        if (success) {
            fetchData();
...
const batchDelete = async () => {
    ...
    if (ids.length > 0) {
        try {
            ...
            const { success } = await post(`/operationLog/batchDeleteByIds`, { ids: ids });
            if (success) {
                fetchData();
                ElMessage.success('Deleted successfully');
            }
